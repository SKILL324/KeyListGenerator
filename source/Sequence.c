...
void Sort_Words(FILE* hFile, int number_words, int sequence_size)
{
	fseek(hFile, 0, SEEK_SET);
	char* buffer = (char*)calloc(sequence_size,sizeof(char));
	static char** file_words;

	if (file_words = (char**)calloc(number_words + 1, sizeof(char*)))
	{
	...
	{
		fprintf(stdout, "Error file_words calloc!\n");
		return;
	}
	...
	FILE* aFile;
	if (!(aFile = CreateFile("Sequence_Sorted.txt", "w")))
	{
	...
	while(file_words[i] != NULL)
	{
		if (file_words[i + 1] != NULL)
		{
			if (strcmp(file_words[i], file_words[i + 1]) != 0)
				FileWriteLine(file_words[i], aFile);
		}
		else
		{
			FileWriteLine(file_words[i], aFile);
